...
class FederationAgent:
    ...
    def _create_demo_secure_content(self) -> None:
        """
        Populate the secure store with demo folders and placeholder files.

        These represent the kind of sensitive data that would reside on the
        encrypted partition in a real deployment (network reports, node
        configuration, access logs).  Files are only created once; existing
        ...
        demo_structure = {
            "reports": {
                "network_summary.txt": (
                    f"Node: {self.node_id}\n"
                    "Status: SECURE PARTITION ACTIVE\n"
                    "Encryption: AES-256-GCM (demo) / LUKS AES-256 (production)\n"
                    "Key Exchange: RSA-2048 + XOR shard scheme\n"
                    "This file resides on the encrypted secure partition.\n"
                    "In a real deployment it would contain sensitive network\n"
                    "telemetry and audit summaries.\n"
                ),
                "device_inventory.txt": (
                    "# Authorised Device Inventory\n"
                    "# Generated by the Captive Portal on first mount.\n"
                    "# Real entries are managed through the Admin Panel.\n"
                ),
            },
            "configs": {
                "node_config.json": json.dumps(
                    {
                        "node_id": self.node_id,
                        "encryption": "AES-256",
                        "key_scheme": "XOR-N-of-N",
                        "federation_enabled": True,
                        "note": "Sensitive configuration — keep this partition encrypted.",
                    },
                    indent=2,
                ),
            },
            "logs": {
                "access.log": (
                    "# Secure Partition Access Log\n"
                    "# Each line records when the partition was successfully mounted.\n"
                    f"[BOOT] Node {self.node_id} mounted secure partition.\n"
                ),
            },
        }

        for folder, files in demo_structure.items():
        ...
        logger.info("Demo secure content initialised in %s", self._secure_dir)
